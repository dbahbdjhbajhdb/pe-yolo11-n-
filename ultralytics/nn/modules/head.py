...
__all__ = "OBB", "Classify", "Detect", "Pose", "RTDETRDecoder", "Segment", "YOLOEDetect", "YOLOESegment", "v10Detect", "RDC_Head", "R3Head"
...
class RDC_Head(Detect):
    """Lightweight residual detection head kept under the RDC_Head name."""
    ...
    def __init__(self, nc=80, ch=()):
        ...
        # Shared refiners keep the accuracy-oriented residual path with very few extra parameters.
        self.reg_refine = nn.Sequential(DWConv(c2, c2, 3), Conv(c2, c2, 1))
        self.cls_refine = nn.Sequential(DWConv(c3, c3, 3), Conv(c3, c3, 1))
        self.reg_scale = nn.Parameter(torch.tensor(0.1))
        self.cls_scale = nn.Parameter(torch.tensor(0.1))
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            reg = self.cv2[i][0](x[i])
            reg = reg + self.reg_scale * self.reg_refine(reg)
            reg = self.cv2[i][2](self.cv2[i][1](reg))

            cls = self.cv3[i][0](x[i])
            cls = cls + self.cls_scale * self.cls_refine(cls)
            cls = self.cv3[i][2](self.cv3[i][1](cls))
